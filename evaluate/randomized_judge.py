...
def call_judge(api_type: str, judge_client, judge_model: str, prompt: str, response1: str, response2: str, allow_ties: bool, randomize_order: bool = True) -> Dict[str, Any]:
    ...
    if allow_ties:
        ...
        comparison_prompt = (
            ...
            "Please provide:\n" 
            "1. Which response is clearly better (A or B), if you cannot clearly determine which is better, give a tie\n" # clearly better, otherwise give a tie
        ...
        #     "- Clarity and coherence\n"
        #     "- Helpfulness and usefulness\n\n"
        #     "- Do not penalize the responses for being truncated, but if the response\n\n"
        #     "Please provide:\n"
...
def main():
    parser = argparse.ArgumentParser(description="Judge from a saved completions artifact")
    ...
    for idx, it in enumerate(tqdm(items, desc="Judging with majority voting")):
        prompt = it["prompt"]
